<div class="btp-dropdown" data-behaviour="btp-dropdown-filter">
  ...
  <ul class="btp-dropdown-menu select<?php if( isset( $args['class'] )&& $args['class'] ){ _e( " ".$args['class'] ); }?>" data-field-type="select" role="menu" aria-labelledby="selection menu for <?php _e( $args['name']."_default" );?>">
    <li class="btp-author-search">
      <input type="text" placeholder="Search Author...." data-behaviour="btp-author-typeahead" autofocus autocomplete="off">
    </li>
    <li>
    ...
    <li class="checkbox">
      <input type="radio" name="<?php _e( $args['name'] );?>" id="<?php _e( $item['slug'] ); ?>"
      data-author = "<?php echo strtolower( ucwords( str_replace( "-", " ", $item['name'] ) ) );?>"
      value="<?php _e( $item['slug'] ); ?>" <?php if( isset( $args['value'] ) && $item['slug'] == $args['value'] ){_e("checked");}?>>
